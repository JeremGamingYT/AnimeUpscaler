...
from anime_upscaler.data.dataset import AnimeSRDataset, list_images
...
def main(cfg: TrainConfig | None = None) -> None:
    ...
    if conf["train"].get("compile", False):
        try:
    ...
    while step < total_steps:
        for batch in loader:
            ...
            if step % 20 == 0:
                ...
                if use_gan:
                    writer.add_scalar("loss/gan_d", loss_d.item(), step)
                try:
    ...
    try:
        ...
        # Bicubic upscale of LR to HR size for comparison
